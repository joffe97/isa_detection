...
class ClassifierHyperparamsComparison(ResultSaver):
    def __init__(self, title: str = "Hyperparameters comparison") -> None:
        super().__init__()
        self.title = title
    ...
    def save_in_directory(self, isa_model_info_collection: ISAModelInfoCollection) -> None:
        ...
        for isa_model_info in isa_model_info_collection.collection:
            ...
            c = configuration.classifier.__dict__.get("C")
            ...
            c_precision_mapping = classifier_precision_mapping.setdefault(readable_classifier_str, dict())
            c_precision_mapping[c] = mean_precision

        # bar_width = 0.2

        for plot_identifier, feature_mapping in sorted(plot_identifier_feature_mapping.items()):
            for feature_str, classifier_mapping in sorted(feature_mapping.items()):
                _, ax = plt.subplots(figsize=(9, 6))
                for classifier_str, c_precision_mapping in sorted(classifier_mapping.items()):
                    # index = np.arange(len(classifier_mapping))
                    c_list, precisions = list(zip(*sorted(c_precision_mapping.items())))
                    # if feature_i == 0:
                ...
                fig_path = self._filepath_for_identifier(".png", f"{plot_identifier}_{feature_str}")
