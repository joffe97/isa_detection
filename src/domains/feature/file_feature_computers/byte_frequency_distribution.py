...
class ByteFrequencyDistribution(FileFeatureComputer):
    ...
    def compute(self, binary_file: str) -> dict[str, FeatureEntry]:
        ...
        byte_frequency_distribution_f64 = byte_frequency_distribution_u64.astype(np.float64)
        byte_frequency_distribution_f64 /= sum(byte_frequency_distribution_f64)
        return dict(
            (f"bfd_{i}", FeatureEntry(value, i)) for i, value in enumerate(byte_frequency_distribution_f64)
        )
