...
class AutoCorrelationPeakComputer(BytesComputer):
    def __init__(
        self, lag_max: int, lag_min: int = 1, n_peaks: int = 1
    ) -> None:
    ...
    def compute(self, data: bytes) -> list[int]:
        autocorr_data = AutoCorrelationComputer(
            self.lag_max, self.lag_min
        ).compute(data)
        enumerated_autocorr_data_sorted_descending = sorted(
            enumerate(autocorr_data), key=lambda x: x[1], reverse=True
        )
        enumerated_autocorr_data_sorted_descending_largest = (
            enumerated_autocorr_data_sorted_descending[: self.n_peaks]
        )
        return [
            index
            for index, _ in enumerated_autocorr_data_sorted_descending_largest
        ]
