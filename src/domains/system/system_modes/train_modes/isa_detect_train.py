...
class ISADetectTrain(TrainMode):
    def run(isa_model_configuration: ISAModelConfiguration) -> ISAModelCollection:
        isa_binary_features = ISABinaryFeaturesPicker(
            isa_model_configuration.feature_computer_container_collection, isa_model_configuration.target_label).isadetect_features_full_binaries(isa_model_configuration.files_per_architecture)
